...
class GeometryTiler(Tiler[SharedMetadata]):
    name = "geometry"
    ...
    def __init__(
        ...
        self.written_tiles_by_parent_id: dict[int, list[TileInfo]] = {}
    ...
    def send_tile_to_write(
        ...
        return GeometryTilerMessage.WRITE_TILE.value, [tile, pickle.dumps(metadata)]

    def send_file_to_read(self, filename: Path) -> tuple[bytes, list[bytes]]:
        if self.verbosity >= 1:
            print(f"Sending {filename} to read to worker")
        return GeometryTilerMessage.READ_FILE.value, [pickle.dumps(filename)]
    ...
    def add_tile_to_index(self, parent_id: int, tile: TileInfo) -> None:
    ...
    def create_root_tile(self, tile_metadata: TileInfo) -> Tile:
    ...
    def process_message(self, message_type: bytes, message: list[bytes]) -> None:
        ...
        if message_type == GeometryWorkerMessage.TILE_PARSED.value:
            ...
            tile_metadata: TileInfo = pickle.loads(message[0])
            if self.verbosity >= 1:
    ...
    def create_child_tile_and_recurse(
        self, current_tile: Tile, child: TileInfo
    ) -> float | None:
        if self.spec_version == SpecVersion.V1_0:
            extension = "b3dm"
        else:
    ...
    def add_children_to_tile(self, current_tile: Tile) -> None:
        ...
        children: list[TileInfo] | None = self.written_tiles_by_parent_id.get(
