...
class GeometryTilerMessage(Enum):
    READ_FILE = b"read_file"
    WRITE_TILE = b"write_tile"


class GeometryWorkerMessage(Enum):
    TILE_PARSED = b"tile_parsed"  # sent when anybody wants something to get processed
    TILE_READY = b"tile_ready"  # sent when a tile content has been written to disk and a tile has been created
