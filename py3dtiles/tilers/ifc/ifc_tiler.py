...
from py3dtiles.tilers.point.node.shared_node_store import SharedNodeStore
...
def _get_parent(element: entity_instance) -> Optional[entity_instance]:
...
class IfcTiler(Tiler[IfcSharedMetadata, IfcTilerWorker]):
    ...
    tiles_to_write: list[FilenameAndTilePath]
    number_of_jobs: int
    verbosity: int
    cache_size: int
    ...
    def __init__(
        ...
        self.tiles_to_write: list[FilenameAndTilePath] = []
    ...
    def get_tasks(self) -> Iterator[tuple[bytes, list[bytes]]]:
        # yield from self.send_elem_to_process()
    ...
    def send_tile_to_write(
        ...
        tile = self.node_store.get(tile_id)
    ...
    def add_tile_to_queue(self, tile_id: int, filename: str, tile: bytes) -> None:
        ...
        self.node_store.put(id_in_bytes, tile)
        self.tiles_to_write.append(
            FilenameAndTilePath(filename=filename, id_in_bytes=id_in_bytes)
        )
        if self.verbosity >= 1:
    ...
    def process_message(self, message_type: bytes, message: list[bytes]) -> None:
        ...
        if message_type == IfcWorkerMessage.TILE_PARSED.value:
            ...
            filename = message[0].decode()
            offset, crs_in = pickle.loads(message[1])
            crs_in = None
            transformer = None
            ...
            self.files_metadata[filename] = FileMetadata(
                offset=offset, crs_in=crs_in, transformer=transformer
            )
        else:
    ...
    def add_children_to_tile(self, current_tile: Tile) -> None:
        ...
        children: Optional[list[IfcTileInfo]] = self.written_tiles_by_parent_id.get(
        ...
        if children is not None:
            for child in children:
                ...
                if (
                    ...
                    geometric_error = max(
                        geometric_error, child.elem_max_size, child_tile.geometric_error
                    )
